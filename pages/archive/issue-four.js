...
import {
    ...
    ISSUE_THREE_MARKER, ISSUE_THREE_CATEGORY_NAME, ISSUE_FOUR_MARKER, ISSUE_FOUR_CATEGORY_NAME,
} from '../../lib/constants';
...
export default function IssueOne({ allPosts: { edges }, preview }) {
  ...
  const issueThreePosts = postsByCategories(edges, [ISSUE_FOUR_MARKER]);
  const route = 'posts'
...
export async function getStaticProps({preview = false}) {
  const allPosts = await getAllPostsByCategoryName(preview, ISSUE_FOUR_CATEGORY_NAME);
  return {
