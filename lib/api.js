...
export async function getAllPostsWithSlug() {
  const data = await fetchAPI(`
    {
      posts(first: 10000, where: { stati: [PUBLISH] }) {
        edges {
          node {
            slug
          }
...
export async function getAllPostsWithSlugByCategoryName(categoryName) {
  const data = await fetchAPI(`
    query AllPostsWithSlugByCategoryName($categoryName: String = "") {
      posts(
      first: 10000
      where: {categoryName: $categoryName, stati: [PUBLISH]}) {
        edges {
          node {
            slug
          }
...
export async function getAllPostsForHome(preview) {
  const data = await fetchAPI(
    `
    query AllPosts {
      posts(first: 100, where: { orderby: { field: DATE, order: DESC }, stati: [PUBLISH] }) {
        edges {
          node {
            title
            excerpt
...
export async function getAllPostsByCategoryName(preview, categoryName) {
  ...
  posts(
    first: 100
    where: {orderby: {field: DATE, order: DESC}, categoryName: $categoryName, stati: [PUBLISH]}
  ) {
    edges {
      node {
        title
